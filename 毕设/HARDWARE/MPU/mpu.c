...
void MPU_Memory_Protection(void)
{
	...
	MPU_Set_Protection(0x24000000,512*1024,2,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个AXI SRAM,共512K字节,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0x30000000,512*1024,3,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个SRAM1~SRAM3,共288K字节,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0x38000000,64*1024,4,MPU_REGION_FULL_ACCESS,0,1,1);		//保护整个SRAM4,共64K字节,禁止共用,允许cache,允许缓冲

	MPU_Set_Protection(0x60000000,64*1024*1024,5,MPU_REGION_FULL_ACCESS,0,0,0);	//保护MCU LCD屏所在的FMC区域,,共64M字节,禁止共用,禁止cache,禁止缓冲
	MPU_Set_Protection(0XC0000000,32*1024*1024,6,MPU_REGION_FULL_ACCESS,0,1,1);	//保护SDRAM区域,共32M字节,禁止共用,允许cache,允许缓冲
	MPU_Set_Protection(0X80000000,256*1024*1024,7,MPU_REGION_FULL_ACCESS,0,0,0);//保护整个NAND FLASH区域,共256M字节,禁止共用,禁止cache,禁止缓冲
}
